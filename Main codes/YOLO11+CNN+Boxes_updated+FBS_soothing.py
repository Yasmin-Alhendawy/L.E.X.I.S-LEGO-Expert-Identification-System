# -*- coding: utf-8 -*-
"""
Created on Mon May 26 10:17:24 2025
...
# === Configuration ===
IMAGE_SIZE = (96, 96)
FRAME_RESOLUTION = (640, 640)  # Width, Height
...
MIN_AREA = int(0.0015 * frame_area)   # ~0.15% of frame
MAX_AREA = int(0.22 * frame_area)     # ~22% of frame
...
while True:
    ...
    if len(filtered_boxes) == 0:
        ...
        for box_info in filtered_boxes:
            ...
            lego_color = find_closest_lego_color((b, g, r))
            text = f"{label} ({conf*100:.1f}%) [{lego_color}] | YOLO: {box_info['score']*100:.1f}%"
            cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
            cv2.putText(frame, text, (x1, y1 - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)

    fps_end_time = time.time()
    current_fps = 1.0 / (fps_end_time - fps_start_time + 1e-5)
    fps_history.append(current_fps)
    smoothed_fps = sum(fps_history) / len(fps_history)

    cv2.putText(frame, f"FPS: {smoothed_fps:.1f}", (10, 60),
