...
def run_on_image(image_path):
    ...
    results = yolo_model.predict(source=image, device=device, conf=0.5, verbose=False)
...
def browse_and_process():
    ...
    if filepath:
        processed_img = run_on_image(filepath)
...
btn_image = tk.Button(root, text="Select Image", command=browse_and_process)
